...
transaction(senderAccount: Address, amount: UFix64) {
    ...
    // Define references
    let senderVault: &SarahToken.Vault{SarahToken.CollectionPublic}
    ...
    let adminResource: &SarahToken.Admin
    let flowMinter: &FlowToken.Minter

    prepare(acct: AuthAccount) {
        // Borrow references and handle errors
        self.adminResource = acct.borrow<&SarahToken.Admin>(from: /storage/AdminStorage)
        ...
        self.senderVault = getAccount(senderAccount)
            ...
            .borrow<&SarahToken.Vault{SarahToken.CollectionPublic}>()
