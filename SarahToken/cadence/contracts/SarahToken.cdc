...
// SarahToken contract: Implements fungible token on Flow blockchain
pub contract SarahToken: FungibleToken {
    ...
    pub resource Vault: FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, CollectionPublic {
        ...
        pub fun deposit(from: @FungibleToken.Vault) {
            let vault <- from as! @SarahToken.Vault
        ...
        // Destroy the vault
        destroy() {
            SarahToken.totalSupply = SarahToken.totalSupply - self.balance
        }
    }
    ...
    pub resource Minter {
        ...
        pub fun mintToken(amount: UFix64): @FungibleToken.Vault {
            SarahToken.totalSupply = SarahToken.totalSupply + amount
            return <-create Vault(balance: amount)
        }
    }
    ...
    init() {
        ...
        self.account.link<&SarahToken.Minter>(/public/Minter, target: /storage/MinterStorage)
